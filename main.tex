...
When subjects are exposed to the same stimuli, their brain activity must exhibit
some shared response. Our goal is to recover this shared response. The shared
...
the model. In the shared response model, orthogonality constraints are imposed
that are not biologically plausible. A popular and more biologically plausible constraint is the
independence of the recovered responses. However, most popular methods using these
constraints are partially heuristic when multiple subjects are involved. In
contrast, we develop a well principled method that comes with statistical
guarantees. In practice our method is able to recover more reliable estimate of
the shared response on fMRI and MEG data than competitors. This work will be presented in
part~\ref{part:mvica}.
...
The model used in MultiViewICA assumes that all subjects are equally different.
...
inter-subject variability in neuro-imaging, and a data augmentation method for fMRI data.

First, in chapter~\ref{ch:fastsrm1} and chapter~\ref{ch:fastsrm1}, we develop an
...
algorithms in big datasets where the number of subjects is on the order of
...
from the activities of a deep neural network exposed to the same movie. The neural network is previously trained to perform action recognition on a large dataset.
...
In this work, we benchmark optimal transport, ridge regression and scale
Procrustes to align the data of two
subjects. Ridge regression seems to be the best method.
