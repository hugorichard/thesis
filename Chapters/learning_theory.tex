...
\subsection{Bias and efficiency of estimators}
...
\begin{prop}[Newton algorithms are equivariant]
  ...
  a Newton algorithm starting from $\xb_0$.
  Consider $\yb_k$ the current estimate of the minimum of $g(\yb) = f(A\yb)$ for any
  invertible matrix $A$ starting from $\yb_0 = A^{-1}\xb_0$.
  Then, $\yb_k = A^{-1} \xb_k$.
\end{prop}
\begin{proof}
...
\end{align}
\end{proof}
Equivariance typically does not hold for gradient descent.
