...
We use the function $f(\cdot)= \log\cosh(\cdot)$, giving the non-linearity \textbf{XXX unclear why you call it non-linearity} $f'(\cdot) = \tanh(\cdot)$.
...
\begin{figure}
  \label{fig:synth}
  \includegraphics[width=0.5\textwidth]{figures/mvica/amari.pdf}
  \captionof{figure}{\textbf{Synthetic experiment}: reconstruction error of the algorithms on data following model~\eqref{eq:ica_model}. \textbf{XXX make it self-contained}}
\end{figure}
...
\textbf{XXX Refer explicitly to Fig \ref{fig:synth}}
...
%
The CLIPS dataset also pertains to the Individual Brain Charting dataset (\cite{ibc}) and reproduces \textbf{XXX}.
%
...
\textbf{XXX in the framework of a PhD manuscript I find it better to dedicate a section to the daatset description (could be in the introduction) to avoid distilling it across experimental chapters.}
...
\end{figure}
 ...
 In Figure~\ref{fig:reconstruction} \textbf{XXX clash in fig labels ?}
...
MultiView ICA yields a consistent and substantial improvement in accuracy compared to other methods on the four datasets. We see a marked improvement on the  sherlock and forrest datasets \textbf{XXX refer explicitly to the figure}. A possible explanation lies in the preprocessing pipeline. Sherlock data undergo a 6~mm spatial smoothing and Forrest data are acquired at a higher resolution (7T vs 3T for other data). This affects the signal to noise ratio.
%
